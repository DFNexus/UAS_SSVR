...
class AuthRBACTests(TestCase):
    ...
    def test_login_returns_token(self):
        # Admin
        response = self.client.post("/auth/login", json={"username": "admin_test", "password": "pass"})
        self.assertEqual(response.status_code, 200)
        self.assertIn("access", response.json())
        
        # Instructor
        response = self.client.post("/auth/login", json={"username": "inst_test", "password": "pass"})
        self.assertEqual(response.status_code, 200)
        self.assertIn("access", response.json())
        
        # Student
        response = self.client.post("/auth/login", json={"username": "student_test", "password": "pass"})
        self.assertEqual(response.status_code, 200)
        self.assertIn("access", response.json())

    def test_admin_endpoint_rejects_student(self):
        headers = self.get_headers(self.student_user)
        response = self.client.post("/categories/", json={"name": "Test Cat"}, headers=headers)
    ...
    def test_admin_endpoint_allows_admin(self):
        ...
        self.assertEqual(response.json()["name"], "Test Cat")
